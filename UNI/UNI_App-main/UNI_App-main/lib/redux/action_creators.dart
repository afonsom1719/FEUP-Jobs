...
ThunkAction<AppState> setJobFilters(
    List<String> newJobFilters, Completer<Null> action) {
  return (Store<AppState> store) {
    List<String> jobFilters = store.state.content['jobFilters'];
    jobFilters = List<String>.from(newJobFilters);
    store.dispatch(SetJobFilter(jobFilters));
    action.complete();
  };
}
